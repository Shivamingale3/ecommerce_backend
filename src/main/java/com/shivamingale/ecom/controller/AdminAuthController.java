...
public class AdminAuthController {
    ...
    public ResponseEntity<AppResponse<Map<String, String>>> requestOtp(@Valid @RequestBody Map<String, String> request) {
        String email = request.get("email");
        if (email == null || email.isBlank()) throw new AppException(HttpStatus.BAD_REQUEST, "Email is required", null);
        return ResponseEntity.ok(AppResponse.success(adminAuthService.requestSignInOtp(email), "OTP sent successfully", HttpStatus.OK));
    }

    @PostMapping("/verify-otp")
    public ResponseEntity<AppResponse<Void>> verifyOtp(@Valid @RequestBody VerifySignInOtpRequest request, HttpServletResponse response) {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<AppResponse<Void>> refresh(@Valid @RequestBody(required = false) RefreshTokenRequest requestBody, HttpServletRequest request, HttpServletResponse response) {
    ...
    private String extractRefreshToken(HttpServletRequest request, RefreshTokenRequest requestBody) {
        ...
        if (requestBody != null && requestBody.getRefreshToken() != null) return requestBody.getRefreshToken();
